...
# Internal setup + support guide: shared inboxes -> lead sources -> workflow, each with its own test. Shown under Tools > Learning > SOPs & Guides.
LEADS_INBOXES_SOPS = [
    {
        "title": "1. Set Up a Shared Inbox (the team and its number)", "summary": "One number, the whole team. The inbox is the single team definition every lead source and workflow hangs off.",
        "department": "all", "category": "leads_inboxes", "is_required_reading": True, "estimated_time": "10 minutes", "difficulty": "beginner",
        "tags": ["shared inbox", "team", "routing", "leads", "setup"],
        "steps": [
            {"order": 1, "title": "How the pieces fit", "description": "Lead provider (webhook, ADF XML or email) -> LEAD SOURCE -> lands in a SHARED INBOX (one number + the team that works it) -> the Response Workflow or Lead Flow decides the intake text, who rings in what order, the after-hours rule and Jessi -> reps' phones ring, first to press 1 (or tap Claim) owns the lead -> the thread lives in Inbox > Team Inbox / Leads until it is closed or handed to the rep's own line.\n\nShared Inbox = the number customers text or call plus the people who work it. Whoever is on the inbox can see, get pinged on and claim the leads that land there. Lead Source = one feed of leads; it points at ONE inbox. Workflow = what happens the second a lead arrives."},
            {"order": 2, "title": "Before you start", "description": "Check these first, they are the reason a rep 'never rings':\n\n• Every rep has a CELL NUMBER on their profile (Tools > Set Up > Team Members). The call ladder rings that number; no cell = that rep is skipped.\n• Every rep has a TEXTING NUMBER (Tools > Set Up > Phone Numbers). Claimed leads graduate to the rep's own line.\n• STORE HOURS are set (Tools > Set Up > Store Profile). The after-hours rule and 'rings at opening' use them.\n• Reps set MY SCHEDULE (Tools > Settings > My Schedule). Off-shift reps are skipped when at least one rep is on shift; if nobody is on shift everyone is pinged so a lead is never dropped.\n• A Twilio number for the inbox that is NOT a rep's personal line.", "link_text": "Open Team Members", "link_url": "/admin/users"},
            {"order": 3, "title": "Setup tab", "description": "Tools > Manage > Inboxes > Create your first inbox (or the + button).\n\n• Name: 'Sales Internet', 'Service BDC'.\n• What it's for (optional): shows on every thread so reps know which hat they are wearing.\n• Number: the Twilio number customers text. Replies always go out from this number while the thread lives in the inbox.\n• Routing for a new customer who texts in: First to claim (everyone pinged, first tap owns it), Take turns (round robin) or Weighted turns. Daily cap per rep (0 = none): once a rep hits the cap today, new customers skip to the next rep.", "link_text": "Open Inboxes", "link_url": "/inboxes"},
            {"order": 4, "title": "Team tab", "description": "Members are grouped ON <STORE NAME> · N. Tap 'Everyone on the team' to check them all, or tap individuals.\n\nSuper admins and org admins can expand 'Show others' and search for people not on this store. Adding someone from there also puts them on this store's team (toast: 'N added to <store>'s team').\n\nTap Create. You land on the Leads tab.", "tip": "If someone is on the inbox they are automatically in the store's rep pool for lead sources and flows. You never have to add the same person in three places again."},
            {"order": 5, "title": "Leads tab: 'Is this team wired up?'", "description": "This checklist tells you whether leads will actually reach the team:\n\n• Number and Team present.\n• Members without a number / without a cell: fix them (step 2) or those people cannot be rung.\n• Lead sources that land here: if it says 'No lead source points here yet', tap 'Point a lead source here' and pick one (or create one, see SOP 2, and choose this inbox).\n• Ringing, per source: everyone / some / others / none / text only. Tap the gold 'Ring everyone on this inbox' on a source to make attempt 1 ring the whole roster. It follows the team, so new hires ring without touching it again.\n• Jessi: whether the assistant is on for this inbox.", "warning": "'No lead source points here yet' means web or ADF leads never reach this team. Point at least one source here."},
            {"order": 6, "title": "Jessi and Closing tabs (optional)", "description": "Jessi tab: Instant first reply (sent the moment a brand-new customer texts this number; blank = nothing automatic), Assistant name, What she should know, Rules she must follow, Hand to a human when.\n\nClosing tab: the tag that ends sharing ('When a contact from this inbox gets this tag, the thread is done being shared', for example Sold) and 'Bridge text from the rep's line' (the customer's first text from the rep's own number so they save it)."},
            {"order": 7, "title": "TEST the inbox (5 minutes)", "description": "1. From your own cell, text the inbox number.\n2. Open the Inbox tab and tap the Team Inbox chip. The thread appears as up-for-grabs (First to claim) or already assigned (Take turns).\n3. If you set an instant first reply, your cell receives it within seconds.\n4. Every member gets a push. Tap Claim on one phone: it disappears from everyone else's grabs list.\n5. Reply from the app. The text arrives on your cell FROM THE INBOX NUMBER, not the rep's line.\n6. The gold 'Rules & team' button in the chip row jumps back to this inbox's Leads tab any time.", "tip": "Pass = thread visible under Team Inbox, first reply received, claim removes it for others, reply comes from the inbox number."},
        ],
    },
    {
        "title": "2. Connect a Lead Source (receive internet leads)", "summary": "One source per feed: Cars.com, website form, Facebook, Zapier. Webhook, ADF/XML or email intake, then a test lead.",
        "department": "all", "category": "leads_inboxes", "is_required_reading": True, "estimated_time": "10 minutes", "difficulty": "beginner",
        "tags": ["lead source", "ADF", "webhook", "zapier", "internet leads"],
        "steps": [
            {"order": 1, "title": "Create the source", "description": "Tools > Leads > Lead Source Config > + (New Lead Source).\n\n• Source name: one per feed ('Cars.com', 'Website Form', 'AutoTrader'). One source per feed keeps Proof (cost per sale) honest.\n• Assign to team: pick the shared inbox from SOP 1. This is the 'lands in' link.\n• Assignment method: Jump Ball, Round Robin or Weighted Round Robin (used when a lead is assigned rather than rung).\n• Create. You land on the source page with its credentials.", "link_text": "Open Lead Source Config", "link_url": "/admin/lead-sources"},
            {"order": 2, "title": "Provider posts ADF/XML directly", "description": "Cars.com, AutoTrader, CarGurus and OEM portals can POST ADF XML straight to us. On the source page tap the ADF / XML URL to copy it and give that URL to the provider. No key is needed."},
            {"order": 3, "title": "Provider emails ADF leads", "description": "Most CRM-style feeds deliver leads by email. On the source page copy the Email Lead Intake URL. Create a free inbound address on CloudMailin or SendGrid Inbound Parse, point it at that URL, then give the provider that email address. ADF XML is auto-extracted from the email body or attachments.", "tip": "Use one inbound address per source so each feed keeps its own stats."},
            {"order": 4, "title": "Website form, JSON or Zapier / Make", "description": "Website or custom dev: use the Webhook URL plus the API Key as an X-API-Key header. Copy the Example Request on the source page. 'phone' is the only required field; name, email and notes are optional.\n\nZapier / Make / any app: Tools > Leads > Connect Zapier / Make, pick the source. The page lists the field names to use as Data keys and shows 'Listening for your test...' until the first post arrives.", "link_text": "Open Connect Zapier / Make", "link_url": "/admin/lead-connect"},
            {"order": 5, "title": "Where it lands and Active", "description": "On the source page:\n\n• LANDS IN SHARED INBOX: switch the inbox any time without recreating the source. 'No inbox (reps below only)' means only the reps checked in the workflow see it.\n• Active toggle: a paused source rejects posts.", "warning": "Reps who are not on the source's inbox and not checked in its workflow cannot see its leads."},
            {"order": 6, "title": "TEST the source (before touching the workflow)", "description": "Fastest: scroll to 'Send a test lead' on the source page. Enter YOUR OWN CELL as the customer phone, leave 'Ring the call ladder too' unchecked for now, tap Send test lead, then tap again to confirm. The contact is tagged 'Test Lead' and stays out of Proof. The result card shows: intake text sent now / held until <time>, Jessi on or off, store open or closed, N reps pushed.\n\nFrom the provider side: send a real test from the portal or Zap, or run the curl from Example Request with \"is_test\": true added to the JSON body.\n\nConfirm it landed: Tools > Leads > Lead Source Queue shows the lead with the first-text status: SENT went out, QUEUED is waiting for store hours or the texting window, FAILED could not deliver (bad number or opt-out).\n\nConfirm the team sees it: Inbox > Leads shows it as WAITING and everyone on the inbox has a push.", "link_text": "Open Lead Source Queue", "link_url": "/admin/internet-leads", "tip": "Pass = lead row in Lead Source Queue, text SENT or QUEUED with a reason, WAITING in Inbox > Leads, pushes received."},
        ],
    },
    {
        "title": "3. Build the Lead Workflow (text, ring, claim)", "summary": "Intake text, who gets pushed, the call ladder, after-hours, returning customers and Jessi. On the source or as a reusable Lead Flow.",
        "department": "all", "category": "leads_inboxes", "is_required_reading": True, "estimated_time": "15 minutes", "difficulty": "intermediate",
        "tags": ["lead flow", "call ladder", "workflow", "after hours", "jessi"],
        "steps": [
            {"order": 1, "title": "Two places, same controls", "description": "On the source: Tools > Leads > Lead Source Config > open the source > Response Workflow section > Save Workflow. Good for a one-off source.\n\nAs a reusable Lead Flow (recommended past one source): Tools > Leads > Lead Flows > Start a new flow (from a proven template or blank). Then on each source tap 'Use a flow from the library' in the Response Workflow section. Edit the flow once and every source using it updates.", "link_text": "Open Lead Flows", "link_url": "/lead-flows"},
            {"order": 2, "title": "Instant intake text", "description": "• Instant Intake Text: sent to the lead the moment it arrives (inside the texting window). Tap a merge field to insert first name, vehicle, rep name or store.\n• What are these leads asking about? (optional): context so Jessi answers in the right lane.\n• After-hours version: used instead of the intake text when the store is closed. Blank = same text.\n• 'Just tried you' text: sent automatically after a call attempt goes unanswered.\n\nWhen the source lands in an inbox the text goes out FROM THE INBOX NUMBER, so the customer's reply comes back to the whole team."},
            {"order": 3, "title": "Who gets the push and how the customer is contacted", "description": "Notify these reps (first to reply claims the lead): everyone checked is pushed the second the lead lands. When the source lives in a shared inbox, everyone on that inbox is pinged too, automatically.\n\nCustomer Contact: Text only (no phones ring; reps claim from the push or the Leads list) or Text + Call (the call ladder rings reps and bridges the first one to press 1 to the customer)."},
            {"order": 4, "title": "The call ladder (Text + Call)", "description": "Each attempt is a rung: who rings, then how long to wait before the next rung.\n\n• Attempt 1: tap the gold 'Everyone on <inbox>' chip to ring the whole inbox team (follows the roster, new hires included) or pick specific reps.\n• Text first, ring after: Same moment / 15 / 30 / 45 / 60 s. The customer sees your text before phones ring. The rung is skipped if a rep already claimed or texted in the meantime.\n• Add attempt 2, 3 with different reps or managers. 'Pick at least one rep or this attempt is skipped.'\n• Caller ID when ringing reps (Lead Flows): the number the rep sees and the customer sees on the bridged call.\n• When nobody answers: after the last rung rings out, alert the managers. The lead always stays in the shared queue.\n• When a rep claims: 'Claim = call me' auto-dials the rep and bridges them; 'Tags to add' runs tag workflows too, so a 'Working' tag can start its campaign.", "warning": "Use a LOCAL caller ID. Toll-free 8xx numbers get flagged by cell carriers and show up as 'Carrier spam filter declined the call'."},
            {"order": 5, "title": "After hours, texting window, returning customers", "description": "After Store Hours: 'Text + Jessi, ring at opening' (default: customer gets the after-hours text, Jessi answers replies, the ladder fires when the store opens) or 'Ring reps anyway'.\n\nTexting Window (TCPA): Federal 8 AM to 9 PM or Strict states 9 AM to 8 PM, in the CUSTOMER'S time zone. Texts outside the window are held and sent at window start.\n\nReturning Customers: a lead whose number already belongs to one of your reps goes straight to that rep (text only, no ring). 'Quiet for N days' (14 / 30 / 60 / 90): if that rep has no sale and nothing happened in N days, the lead goes back to the team and the old rep gets a quiet 'lead reopened' note. 'Always their rep' turns this off."},
            {"order": 6, "title": "Jessi, queue timers, save", "description": "• Enable AI Auto-Reply (Jessi): she answers the customer's replies until a human claims. Optional VA Profile for this source and Custom Instructions.\n• Lead Queue Timers: turns amber after / turns red after / alert managers after / release to queue after (an unclaimed or ignored lead is released back to everyone).\n• Daily Manager Report toggle.\n\nTap Save Workflow (or Save on the flow)."},
            {"order": 7, "title": "TEST the workflow (10 minutes, two phones if you can)", "description": "1. On the source page, Send a test lead with YOUR CELL as the customer and 'Ring the call ladder too' checked. Tap twice to confirm.\n2. Within seconds your cell gets the intake text (or the card says it is held until the window or opening) and every rep in attempt 1 gets a push and their cell rings after the ring delay you set.\n3. On a rep's phone, answer and press 1. That rep is bridged to your cell (the 'customer'). Everyone else's ring stops.\n4. Reply from your cell. The reply lands in the thread; Jessi stays quiet because a human owns it.\n5. Open the thread in Inbox > Leads. The lead timeline at the top shows every step: text sent, who rang at what time, who claimed, 'Text first, phones ring 30 s later', or '<Rep>'s customer, sent straight to <Rep>' for returning customers.\n6. Let a second test ring out with nobody answering: confirm the 'Just tried you' text arrives, the ladder moves to attempt 2 and managers get the alert after the last rung.\n7. Tools > Leads > Call Retries shows the retry timing report; Lead Source Queue shows the text statuses.\n8. Re-sending a test lead for the same number later flips the thread back to WAITING as a brand-new lead. That is expected.", "link_text": "Open Internet Leads", "link_url": "/leads", "tip": "Pass = intake text on your cell, attempt-1 phones ring, press 1 bridges and stops the others, timeline shows each step, no-answer path sends 'Just tried you' and escalates."},
        ],
    },
    {
        "title": "4. Test Matrix and Troubleshooting (leads and inboxes)", "summary": "Every flow on one page: what to do, what to expect, where to confirm. Plus the fix for each common symptom.",
        "department": "all", "category": "leads_inboxes", "is_required_reading": False, "estimated_time": "5 minutes", "difficulty": "beginner",
        "tags": ["testing", "troubleshooting", "leads", "inbox", "support"],
        "steps": [
            {"order": 1, "title": "Inbox tests", "description": "• Inbox texting: text the inbox number from your cell. Expect the thread under Inbox > Team Inbox, the instant reply, members pushed.\n• Inbox claim: tap Claim on one phone. Expect it gone from other phones' grabs list; replies go out from the inbox number (check the sender on your cell).\n• Inbox checklist: open the inbox > Leads tab. Expect everything green and every source showing who rings.", "link_text": "Open Inboxes", "link_url": "/inboxes"},
            {"order": 2, "title": "Source tests", "description": "• Source intake: Send a test lead with the ladder off. Expect the result card (text sent or held, N reps pushed) and the row in Lead Source Queue as SENT or QUEUED.\n• Provider connection: real test from the portal, Zap or curl. Expect a lead row tagged with the source; the Connect Zapier / Make page stops 'Listening'.", "link_text": "Open Lead Source Queue", "link_url": "/admin/internet-leads"},
            {"order": 3, "title": "Workflow tests", "description": "• Ladder ringing: test lead with the ladder on. Expect attempt-1 phones to ring after the delay; the thread timeline lists each ring.\n• Press 1 claim: answer and press 1. Expect the bridge to the customer and the other rings to stop; the timeline shows the claim.\n• No-answer path: let it ring out. Expect the 'Just tried you' text, the next attempt, then the manager alert.\n• After hours: send a test while the store is closed. Expect the after-hours text, Jessi replying, the ladder held until opening ('rings at <time>' on the result card).\n• Returning customer: test from a number a rep already owns. Expect it to go straight to that rep with no ring; timeline says '<Rep>'s customer'."},
            {"order": 4, "title": "A rep never rings / 'Members without a cell'", "description": "No cell on their profile, they are not in the attempt, or they are not on the inbox team. Add the cell (Tools > Set Up > Team Members), use 'Everyone on <inbox>' on attempt 1, and check the inbox Team tab. The checklist clears itself."},
            {"order": 5, "title": "Text shows QUEUED, or text went out but nothing rang", "description": "QUEUED: outside the customer's texting window, or the store is closed with the default after-hours rule. Normal; it sends at window start or opening. Use 'Ring reps anyway' if you want calls after hours.\n\nText out but no ring: the source is Text only, or the ladder has no reps. Switch Customer Contact to Text + Call and fill attempt 1."},
            {"order": 6, "title": "Wrong people, rejected posts, night pushes", "description": "• Reps outside the source's list cannot see the lead: the source is not pointed at their inbox. Set LANDS IN SHARED INBOX on the source.\n• Leads skip the team and go to one rep: the returning-customer rule (the number already belongs to that rep). Expected; lower 'quiet for N days' if you want stale ones back to the team sooner.\n• Provider posts rejected: the source is paused or the X-API-Key is wrong or missing. Turn the source Active; copy the key from the source page.\n• A rep gets pushes at night: they have no My Schedule. Have them set work hours and quiet times under Tools > Settings > My Schedule."},
            {"order": 7, "title": "'Carrier spam filter declined the call'", "description": "The number ringing reps (or the shop number) is toll-free (800 / 833 / 844 / 855 / 866 / 877 / 888) or has been flagged. Cell carriers block those before the phone rings. Switch the caller ID to a local number. Until Number Health / Voice Integrity registration ships, spot-check numbers on freecallerregistry.com.", "warning": "This is the carrier refusing the call, not a busy phone. Retrying from the same number will fail the same way."},
        ],
    },
]
...
NAVIGATION_MAP = (
    ...
    "LEADS + INBOXES: Tools > Manage > Inboxes (create inbox: Setup / Team / Leads / Jessi / Closing tabs; Leads tab = 'Is this team wired up?' checklist, Point a lead source here, Ring everyone on this inbox). Tools > Leads > Lead Source Config (Webhook URL, ADF / XML URL, Email Lead Intake, API Key, LANDS IN SHARED INBOX, Response Workflow, Send a test lead). Tools > Leads > Lead Flows (reusable playbooks, attach via 'Use a flow from the library'). Tools > Leads > Connect Zapier / Make. Tools > Leads > Lead Source Queue (text status SENT / QUEUED / FAILED). Inbox tab > Team Inbox chip > gold 'Rules & team'. Inbox > Leads = WAITING queue; reps press 1 on the ringing call or tap Claim.\n"
)
...
def knowledge_text() -> str:
    ...
    parts.append("\n=== CHECKLISTS ===")
    for sop in GETTING_STARTED_SOPS + LEADS_INBOXES_SOPS:
...
async def apply_training_refresh(db) -> bool:
    ...
    await db.sops.delete_many({"category": {"$in": ["getting_started", "leads_inboxes"]}})
    for sop in GETTING_STARTED_SOPS + LEADS_INBOXES_SOPS:
