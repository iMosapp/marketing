...
def last_week(client: dict, now: Optional[datetime] = None) -> tuple:
    ...
    label = f"{monday.strftime('%b')} {monday.day} to {(this_monday - timedelta(days=1)).strftime('%b')} {(this_monday - timedelta(days=1)).day}"
...
async def week_shops(db, client: dict, start, end) -> list:
    ...
    for c in calls:
        ...
        rows.append({"name": c.get("rep_name") or "Unknown", "department": ind.dept_label(c.get("department") or "sales"), "script": c.get("script_title") or "Shop", "status": c.get("status"),
...
def digest_line(rows: list, label: str) -> str:
...
def digest_html(client: dict, rows: list, line: str, label: str, url: str, logo_src: str) -> str:
    ...
    table = "".join(trs) or '<tr><td style="padding:12px 6px;color:#777;font-size:14px">Nothing to show for this week.</td></tr>'
    return f"""<div style="font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,sans-serif;max-width:600px;margin:0 auto;padding:20px;background:#f5f3ee">
  <div style="background:#fff;border-radius:18px;overflow:hidden;border:1px solid #e6e1d6">
    <div style="text-align:center;padding:26px 20px 14px;border-bottom:1px solid #eee">{logo}
      <p style="margin:10px 0 0;font-size:11px;letter-spacing:2px;color:#C9A962;font-weight:800">I'M ON SOCIAL · WEEKLY SHOP DIGEST</p>
    </div>
    <div style="padding:26px 30px">
      <h1 style="font-size:20px;line-height:1.3;margin:0 0 6px;color:#111">{ms._esc(client.get('name'))}: {ms._esc(label)}</h1>
      <p style="font-size:15px;line-height:1.65;margin:0 0 14px;color:#1a1a1a">Morning {ms._esc(first)}. {ms._esc(line)}</p>
      <table role="presentation" style="width:100%;border-collapse:collapse;margin:0 0 6px">{table}</table>
      <p style="margin:22px 0 10px;text-align:center"><a href="{url}" style="background:#C9A962;color:#111;text-decoration:none;font-weight:800;padding:14px 26px;border-radius:12px;display:inline-block;font-size:15px">Hear the calls and read the coaching</a></p>
      <p style="font-size:12.5px;color:#666;line-height:1.6;margin:0">This is the quick Monday loop. The full monthly PDF still arrives on the 1st if you have it on. Reply to this email with any questions.</p>
    </div>
  </div>
  <p style="text-align:center;margin:18px 0 0;color:#999;font-size:12px">I'm On Social LLC · 1741 Lunford Ln, Riverton, UT 84065 · You get this because the weekly digest is on for {ms._esc(client.get('name'))}.</p>
</div>"""
...
async def send_weekly_digest(db, client: dict, to: Optional[str] = None, actor: Optional[dict] = None, reason: str = "manual", now: Optional[datetime] = None) -> dict:
    ...
    sender = os.environ.get("SENDER_EMAIL", "notifications@send.imonsocial.com")
    payload = {"from": f"I'm On Social <{sender}>", "to": [to], "reply_to": (actor or {}).get("email") or os.environ.get("REPORT_REPLY_TO", "support@imonsocial.com"),
               "subject": f"{client.get('name')}: last week's mystery shops ({label})", "html": digest_html(client, rows, line, label, url, "cid:imos-logo"), "text": f"{line}\n\nOpen the live report: {url}"}
    logo = ms.logo_b64()
    if logo:
