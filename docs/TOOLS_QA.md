...
- June 2026: Phone-viewport sweep (390x844, all 88 routes in this file) after Batch 9. Found and fixed: Help Center + Training Hub + Phone Numbers still had custom headers (never in a batch) -> shared header; Invite Team first/last name inputs and Forecast inputs overflowed the screen (RN-web TextInput needs minWidth 0 on flex parents); Accounts long store names not truncated; bare empty-string children in Individuals (646 console errors), Shared Inboxes and Quick Send; SOPs missing list key. False positives to ignore in future sweeps: horizontal ScrollView pills (My Numbers, AI Outreach, My VA), the Admin Dashboard ticker, and /help on web (marketing page, not the app screen).
