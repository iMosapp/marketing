...
type Props = { hours: Hours; timezone: string; timezones: TZ[]; onHours: (h: Hours) => void; onTimezone: (tz: string) => void };

// GM picks the window we may call in: days, open, close, time zone. Chips, no typing.
export const KickoffHours = ({ hours, timezone, timezones, onHours, onTimezone }: Props) => {
  ...
  return (
    <View style={{ gap: 14 }}>
      ...
      <Text style={{ fontSize: 13, color: LIGHT.textSecondary, lineHeight: 18 }} {...({ testID: 'kickoff-hours-summary' } as any)}>Our shopper calls {DAYS.filter((_, i) => hours.days.includes(i)).join(', ') || 'no days yet'}, between {fmtHour(hours.start)} and {fmtHour(hours.end)} {timezones.find(t => t.id === timezone)?.label || ''} time, never all at once.</Text>
    </View>
  );
};
