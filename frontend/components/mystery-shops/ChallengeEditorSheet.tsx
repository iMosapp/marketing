...
import { Sheet, Field, Label, Chip, GoldButton, DEPTS, GOLD, PURPLE, tid, type Challenge, type ChallengeDraft } from './shared';
...
export const ChallengeEditorSheet = ({ visible, onClose, colors, initial, existing, scope, onSaved }: Props) => {
  ...
  useEffect(() => {
    if (!visible) return;
    setF(existing ? toForm(existing) : initial ? toForm(initial) : blank);
