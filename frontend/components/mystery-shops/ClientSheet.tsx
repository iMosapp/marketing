...
const blank = { name: '', industry: 'automotive', brand: '', city: '', state: '', contact_name: '', contact_email: '', contact_phone: '', contact_title: '', per: { sales: '20', service: '20' } as Record<string, string>, price: '400', start: '09:00', end: '18:00', days: [0, 1, 2, 3, 4, 5], vehicles: '', from_number: '', record: true, notes: '', timezone: 'America/Denver' };
...
export const ClientSheet = ({ visible, onClose, colors, client, onSaved, defaultFrom }: Props) => {
  ...
  useEffect(() => {
    ...
    if (client) setF({ name: client.name, industry: client.industry || 'automotive', brand: client.brand, city: client.city, state: client.state, contact_name: client.contact_name, contact_email: client.contact_email, contact_phone: client.contact_phone, contact_title: client.contact_title,
  ...
  const save = async () => {
    ...
    try {
      ...
      const payload = { name: f.name, industry: f.industry, brand: f.brand, city: f.city, state: f.state, timezone: f.timezone, contact_name: f.contact_name, contact_email: f.contact_email.trim(), contact_phone: f.contact_phone, contact_title: f.contact_title,
    ...
    <Sheet visible={visible} onClose={onClose} title={client ? 'Edit client' : 'New mystery shop client'} colors={colors} testID="client-sheet" footer={<GoldButton label={client ? 'Save changes' : 'Add client'} onPress={save} busy={busy} testID="client-save" />}>
      ...
      <View style={{ gap: 8 }}>
        ...
        <View style={{ flexDirection: 'row', gap: 10, flexWrap: 'wrap' }}>
          ...
          <View style={{ flex: 1, minWidth: 110 }}><Field label="PRICE ($/MO)" value={f.price} onChange={(v: string) => set('price', v.replace(/[^\d.]/g, ''))} colors={colors} keyboardType="decimal-pad" testID="client-plan-price" /></View>
        </View>
      </View>
      <View style={{ gap: 8 }}>
