...
import { Sheet, Label, Chip, GoldButton, DEPTS, deptLabel, GOLD, PURPLE, tid, type Challenge } from '../mystery-shops/shared';
...
export const ChallengePickerSheet = ({ visible, onClose, colors, already, defaultDept, onAdd }: { visible: boolean; onClose: () => void; colors: any; already: string[]; defaultDept?: string; onAdd: (ids: string[]) => Promise<void> }) => {
  ...
  useEffect(() => { if (visible) { setPicked([]); setQ(''); setDept(defaultDept && defaultDept !== 'mixed' ? defaultDept : 'all'); api.get('/shop-clients/challenges').then(r => setRows(r.data.challenges)).catch(() => setRows([])); } }, [visible]);
  const list = (rows || []).filter(c => !already.includes(c.id) && (dept === 'all' || c.department === dept) && (!q.trim() || c.title.toLowerCase().includes(q.toLowerCase()) || (c.purpose || '').toLowerCase().includes(q.toLowerCase())));
  ...
  return (
    <Sheet visible={visible} onClose={onClose} title="Add challenges" colors={colors} testID="picker-sheet" footer={<GoldButton label={picked.length ? `Add ${picked.length} to the course` : 'Pick challenges'} onPress={async () => { setBusy(true); try { await onAdd(picked); onClose(); } finally { setBusy(false); } }} busy={busy} disabled={!picked.length} testID="picker-add" />}>
      ...
      {rows === null ? <ActivityIndicator color={GOLD} /> : list.length === 0 ? <Text style={{ fontSize: 13, color: colors.textSecondary }}>Nothing left to add here. Write more in the Challenge Library.</Text> : list.map(c => {
