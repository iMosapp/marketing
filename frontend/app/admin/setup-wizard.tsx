...
export default function SetupWizardScreen() {
    ...
    <View>
      ...
      <SectionCard>
        ...
        <TouchableOpacity style={[s.input, { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' }]}
          ...
          <Text style={{ fontSize: 18, color: industry ? colors.text : colors.textTertiary }}>{industry || 'Select industry...'}</Text>
        ...
        <TouchableOpacity style={s.logoBox} onPress={uploadLogo}>
          {logoUploading ? <ActivityIndicator size="large" color="#C9A962" /> :
           ...
           <View style={{ alignItems: 'center' }}>
             ...
             <Text style={{ color: colors.textSecondary, marginTop: 10, fontSize: 16, fontWeight: '500' }}>Tap to upload logo</Text>
             <Text style={{ color: colors.textTertiary, fontSize: 14, marginTop: 2 }}>PNG, JPG up to 5MB</Text>
           </View>}
        </TouchableOpacity>
      </SectionCard>
      <SectionCard>
    ...
    <View>
      ...
      <SectionCard style={{ padding: 12 }}>
        <View style={s.rosterHeader}>
          ...
          <Text style={{ flex: 1, fontSize: 15, color: colors.textSecondary, marginLeft: 8, lineHeight: 18 }}>
        ...
        <SectionCard key={idx} style={m.createdId ? { borderColor: '#34C759', borderWidth: 1.5 } : m.error ? { borderColor: '#FF3B30', borderWidth: 1.5 } : undefined}>
          <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', marginBottom: 8 }}>
            <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8 }}>
              <View style={[s.memberBadge, m.createdId ? { backgroundColor: '#34C75920' } : {}]}>
                {m.createdId ? <Ionicons name="checkmark-circle" size={16} color="#34C759" /> : <Text style={{ fontSize: 15, fontWeight: '700', color: colors.textSecondary }}>{idx + 1}</Text>}
              </View>
          ...
          {m.createdId ? (
            <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8, padding: 8, backgroundColor: '#34C75910', borderRadius: 8 }}>
              ...
              <Text style={{ flex: 1, fontSize: 15, color: '#34C759' }}>Created! Password: <Text style={{ fontWeight: '700', fontFamily: Platform.OS === 'web' ? 'monospace' : undefined }}>{m.tempPassword}</Text></Text>
            </View>
          ) : (
            <View>
              {m.error && (
                <View style={{ flexDirection: 'row', alignItems: 'center', gap: 6, marginBottom: 8, backgroundColor: '#FF3B3015', padding: 8, borderRadius: 8 }}>
                  ...
                  <Text style={{ fontSize: 14, color: '#FF3B30' }}>{m.error}</Text>
                </View>
              )}
      ...
      {!bulkCreated && (
        <TouchableOpacity style={s.addMemberBtn} onPress={addMember}>
          ...
          <Text style={{ fontSize: 17, fontWeight: '600', color: '#C9A962' }}>Add Another Team Member</Text>
        </TouchableOpacity>
      )}
      ...
      {bulkCreating && (
        <View style={{ alignItems: 'center', paddingVertical: 16 }}>
          ...
          <Text style={{ color: colors.textSecondary, marginTop: 8, fontSize: 16 }}>Creating accounts...</Text>
        </View>
      )}
    </View>
  );
          ...
          <TouchableOpacity key={i} onPress={() => {
            ...
            <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' }}>
              <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8, flex: 1 }}>
                ...
                <Text style={{ fontSize: 17, fontWeight: '600', color: colors.text }}>{t.name}</Text>
              </View>
              <Ionicons name={t.enabled ? 'checkmark-circle' : 'ellipse-outline'} size={24} color={t.enabled ? '#34C759' : colors.textTertiary} />
            </View>
            <Text style={{ fontSize: 15, color: colors.textSecondary, marginTop: 6, marginLeft: 24 }} numberOfLines={2}>{t.content}</Text>
          </TouchableOpacity>
        ))}
      </SectionCard>
      <SectionCard>
        <Text style={{ fontSize: 16, fontWeight: '600', color: colors.text, marginBottom: 10 }}>Add Custom Template</Text>
        ...
        <TouchableOpacity style={[s.btnSmall, (!newTemplateName.trim() || !newTemplateContent.trim()) && s.btnDisabled]}
          ...
          <Ionicons name="add" size={16} color="#000" /><Text style={{ fontSize: 15, fontWeight: '600', color: '#000' }}>Add Template</Text>
        </TouchableOpacity>
      </SectionCard>
      ...
      <SectionCard>
        ...
        <View style={{ flexDirection: 'row', gap: 8, alignItems: 'center' }}>
          ...
          <TouchableOpacity style={[s.btnSmall, !newTagName.trim() && s.btnDisabled]} onPress={addNewTag} disabled={!newTagName.trim()}>
            <Ionicons name="add" size={16} color="#000" /><Text style={{ fontSize: 15, fontWeight: '600', color: '#000' }}>Create</Text>
          </TouchableOpacity>
        </View>
      </SectionCard>
  ...
  const renderStep7 = () => {
    ...
    return (
      <View>
            ...
            <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', marginBottom: 14 }}>
              ...
              <View style={{ flexDirection: 'row', gap: 8 }}>
                ...
                <TouchableOpacity style={s.actionChip} onPress={() => {
                  ...
                  <Ionicons name="chatbubble-outline" size={14} color="#34C759" /><Text style={{ fontSize: 14, color: '#34C759', fontWeight: '600' }}>Text</Text>
                </TouchableOpacity>
                {Platform.OS === 'web' && (
            ...
            {created.map((m, i) => (
              <View key={i} style={s.credRow}>
                <View style={[s.memberBadge, { backgroundColor: primaryColor + '20' }]}>
                  <Text style={{ fontSize: 14, fontWeight: '700', color: primaryColor }}>{m.firstName[0]}{m.lastName?.[0] || ''}</Text>
                </View>
        ...
        <SectionCard>
          <Text style={[s.label, { fontSize: 16, marginBottom: 10 }]}>Configuration Status</Text>
          {items.map((item, i) => (
            <View key={i} style={s.checklistRow}>
              ...
              <Text style={{ flex: 1, fontSize: 16, color: item.done ? colors.text : colors.textSecondary, fontWeight: item.done ? '500' : '400' }}>{item.label}</Text>
            </View>
          ))}
        </SectionCard>
        ...
        {todos.length > 0 && (
          <SectionCard style={{ borderColor: '#FF9500', borderWidth: 1 }}>
            <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8, marginBottom: 12 }}>
              ...
              <Text style={{ fontSize: 16, fontWeight: '700', color: '#FF9500' }}>Remaining Items</Text>
            </View>
            {todos.map((item, i) => (
              <View key={i} style={{ flexDirection: 'row', alignItems: 'flex-start', paddingVertical: 6, gap: 10 }}>
                <View style={s.todoCheckbox} />
                <Text style={{ flex: 1, fontSize: 16, color: colors.text, lineHeight: 20 }}>{item.action}</Text>
              </View>
            ))}
          </SectionCard>
        )}
  ...
  if (loading) return (
    <SafeAreaView style={[s.container, { justifyContent: 'center', alignItems: 'center' }]}>
      <ActivityIndicator size="large" color="#C9A962" />
      <Text style={{ color: colors.textSecondary, marginTop: 12, fontSize: 16 }}>Loading wizard...</Text>
    </SafeAreaView>
  );
...
const getS = (colors: any) => StyleSheet.create({
  ...
  headerSubtitle: { fontSize: 14, color: colors.textTertiary, marginTop: 1 },
  ...
  dotText: { fontSize: 14, fontWeight: '700', color: colors.textTertiary },
  stepLabel: { fontSize: 12, color: colors.textTertiary, marginTop: 4, textAlign: 'center', fontWeight: '500' },
  ...
  stepNumBadgeText: { fontSize: 16, fontWeight: '800', color: '#000' },
  stepTitle: { fontSize: 22, fontWeight: '800', color: colors.text },
  stepDesc: { fontSize: 16, color: colors.textSecondary, lineHeight: 20, marginLeft: 38 },
  ...
  btnSecondaryText: { fontSize: 17, fontWeight: '600', color: colors.textSecondary },
  btnSkip: { flexDirection: 'row', alignItems: 'center', gap: 4, paddingVertical: 14, paddingHorizontal: 14 },
  btnSkipText: { fontSize: 17, color: '#FF9500', fontWeight: '600' },
