...
export default function ReferralTemplatesDataScreen() {
  ...
  const loadTemplates = async () => {
    try {
      const response = await api.get('/admin/data/referral-templates');
      setTemplates(response.data || []);
    } catch (error) {
      console.error('Failed to load referral templates:', error);
    } finally {
      setLoading(false);
      setRefreshing(false);
    }
  ...
  const renderTemplate = ({ item }: { item: TemplateSent }) => (
    <View style={styles.templateItem}>
      <View style={styles.templateIcon}>
        <Ionicons name="share" size={18} color="#5856D6" />
      </View>
  ...
  return (
    <SafeAreaView style={styles.container} edges={['top']}>
      <View style={styles.header}>
        ...
        <Text style={styles.title}>Referral Templates Sent</Text>
      ...
      <FlatList
        ...
        ListEmptyComponent={() => (
          <View style={styles.emptyContainer}>
            <Ionicons name="share-outline" size={64} color={colors.surface} />
            <Text style={styles.emptyText}>No referral templates sent yet</Text>
          </View>
        )}
      />
    </SafeAreaView>
