  ...
  return (
    <SafeAreaView style={[styles.container, { backgroundColor: colors.bg }]} edges={['top']}>
      <ScrollView ref={scrollRef} contentContainerStyle={styles.scrollContent}>
        ...
        {/* Quick Actions — Top 6 from user preferences */}
        <View style={styles.quickActionsSection}>
          <View style={styles.quickActionsHeader}>
            <Text style={[styles.quickActionsTitle, { color: colors.textSecondary }]}>QUICK ACTIONS</Text>
            <TouchableOpacity onPress={() => setEditingQuickActions(!editingQuickActions)} data-testid="edit-quick-actions">
              <Text style={{ color: colors.accent || '#C9A962', fontSize: 13, fontWeight: '600' }}>
                {editingQuickActions ? 'Done' : 'Edit'}
              </Text>
            </TouchableOpacity>
          </View>
          <View style={styles.quickActionsGrid}>
            {quickActionIds.slice(0, 6).map(id => {
              const action = ALL_ACTIONS.find(a => a.id === id);
              if (!action) return null;
              return (
                <TouchableOpacity
                  key={id}
                  style={[styles.quickActionTile, { backgroundColor: colors.card }]}
                ...
                >
                  ...
                  <Text style={[styles.quickActionLabel, { color: colors.textSecondary }]} numberOfLines={1}>{action.label}</Text>
                </TouchableOpacity>
              );
            })}
          </View>
        ...
        {/* Edit Quick Actions — shows all actions for rearranging */}
        {editingQuickActions && (
          <View style={[styles.editActionsPanel, { backgroundColor: colors.card, borderColor: colors.border }]}>
            <Text style={{ fontSize: 12, color: colors.textSecondary, marginBottom: 12, textAlign: 'center' }}>
              Tap to add or remove. Max 6 Quick Actions.
            </Text>
            <View style={styles.editActionsGrid}>
              {ALL_ACTIONS.map(action => {
                const isSelected = quickActionIds.includes(action.id);
                const position = quickActionIds.indexOf(action.id);
                return (
                  <TouchableOpacity
                    key={action.id}
                    style={[styles.editActionItem, isSelected && { borderColor: action.color, borderWidth: 1.5 }]}
                    onPress={() => toggleQuickAction(action.id)}
                    data-testid={`edit-action-${action.id}`}
                  >
                    <View style={[styles.editActionIconBox, { backgroundColor: `${action.color}18` }]}>
                      <Ionicons name={action.icon as any} size={18} color={action.color} />
                      {isSelected && (
                        <View style={[styles.editActionBadge, { backgroundColor: action.color }]}>
                          <Text style={styles.editActionBadgeText}>{position + 1}</Text>
                        </View>
                      )}
                    </View>
                    <Text style={[styles.editActionLabel, { color: isSelected ? colors.text : colors.textSecondary }]} numberOfLines={1}>
                      {action.label}
                    </Text>
                  </TouchableOpacity>
                );
