...
import {
  View, Text, TouchableOpacity, StyleSheet, Platform, Linking, AppState, ActivityIndicator,
} from 'react-native';
...
export default function CallScreen() {
  ...
  const endCall = async () => {
    ...
    // Cancel the Twilio call if it hasn't connected yet
    if (callSid) {
      try {
        await api.post(`/webhooks/twilio/call-cancel`, { call_sid: callSid }).catch(() => {});
      } catch { /* non-fatal */ }
    }
  };
        ...
        <View style={st.topSection}>
          ...
          {callState === 'ringing' && (
            <View style={st.statusPill}>
              ...
              <Text style={st.statusText}>Your phone is ringing</Text>
            </View>
          )}
          {callState === 'ended' && (
          ...
          {statusMsg ? (
            <Text style={[st.readyHint, { marginTop: 12, textAlign: 'center', paddingHorizontal: 20 }]}>{statusMsg}</Text>
          ) : null}
        </View>
        ...
        <View style={st.bottomSection}>
          {callState === 'ready' && (
            <>
              ...
              <Text style={st.callBtnLabel}>Tap to call via Twilio</Text>
              ...
              <View style={st.loggedCard}>
                ...
                <View style={st.loggedInfo}>
                  ...
                  <Text style={st.loggedDetail}>
                    {seconds > 0 ? `Duration: ${formatTime(seconds)}` : 'Call to'} {contactName}
                  </Text>
                </View>
              </View>
...
const getST = (colors: any) => StyleSheet.create({
  ...
  timer: { fontSize: 48, fontWeight: '300', color: colors.text, fontVariant: ['tabular-nums' as any], letterSpacing: 2 },
  readyHint: { fontSize: 14, color: '#8E8E93', marginTop: 4 },
