...
export const useAuthStore = create<AuthState>((set, get) => ({
  ...
  updateUser: (updates) => {
    set((state) => {
      ...
      // Persist to IDB and AsyncStorage so settings survive app restarts
      // AsyncStorage key MUST be 'user' — that's what loadAuth reads from
      if (updatedUser) {
  ...
  login: async (email, password) => {
    ...
    try {
      ...
      // Push notification registration is handled by _layout.tsx useEffect
      // (fires when isAuthenticated changes — works for both login and session restore)
    } catch (error) {
      set({ isLoading: false });
      throw error;
    }
