...
- **Live shop calls**: `python -m pytest tests/test_live_shops.py` (5, fake Twilio + fake GPT-Live, no network). Test Lab flag `live_shop_calls` is `lab`; per-client chips `client-live-calls-lab|on|off` on English clients (QA Jeep 979a is left on "Follow Test Lab" = null). A real GPT-Live shop call needs OPENAI_API_KEY + a live English shop (500-555 people fail before the stream, so this can only be heard in production).
