...
- **Interview on GPT-Live (Sep 2026)**: `python -m pytest tests/test_live_interview.py` (4, fake Twilio + fake GPT-Live). Test Lab flag `live_interview` defaults to live; with no key the `/api/interview/call/twiml/{sid}?t=` webhook returns ConversationRelay and stamps `interview_sessions.live_transport relay` + `live_skip_reason`. Real call needs OPENAI_API_KEY + Twilio (production only).
- **Who mentioned X (Sep 2026)**: `python -m pytest tests/test_memory_search.py` (4, seeds + wipes "QA Mentions" data for forest; real LLM). `POST /api/memory/search {"query": "who asked about a 20k Tesla Model 3"}` with a bearer token; screen `/memory-search?q=...` (Contacts header sparkles icon). Voice: "someone asked about a 20k Tesla last month, who was it?".
- **Day walkthrough (Sep 2026)**: `python -m pytest tests/test_day_walkthrough.py` (3, seeds + wipes "QA Walk" data for activation-tester). UI: `walk-my-day-btn` inside DO THIS NEXT on Home, only when live config is available+configured; in preview mock `GET /api/live-voice/config` and `POST /api/live-voice/session` (include `open` + `greet_instruction`) and `/delegate` -> `{tool: 'next_stop', open: {...}}`. Voice: "next" / "skip him" / "done" / "draft it".
