...
- Backend tests: `cd /app/backend && set -a && . ./.env && set +a && python -m pytest tests/test_live_voice.py` (4: config/prompt, fake-OpenAI session create, real LLM brain flow on Activation Tester's Sarah/Mike/Dana Tester contacts incl. a text to +15005550042 and a reminder task that is cleaned up, events + admin API). `tests/test_live_voice_api_iter332.py` (testing agent) covers the API guards.
