...
- Industry packs: `GET /api/shop-clients/industries` (admin) lists the 7 packs + per-department challenge counts. `POST /api/shop-clients/challenges/seed {industry, department?}` makes Jessi write 2 starters per empty department (LLM, ~30-60s per department).
