...
- Marketing nav: "Featured" group added at the top of the Solutions dropdown on all 40 marketing pages linking to /relationship-os/ ("The Relationship OS" + PRESENTATION badge), plus mobile Solutions submenu entry on all 19 pages that have it. Script-applied; verified via local http.server screenshot. Needs Vercel redeploy.

## Contact detail restructure (June 2026) - verified by screenshots (web, mobile viewport)
- New order: hero -> health badge -> quick actions (Text, Call, Email, Note, Task, + gold SOLD button when not tagged Sold) -> one-line IntelTeaser ("Jessi's brief: <next step> · N voice notes", taps to Details) -> Feed/Details/Calls tabs.
- Details tab now starts with the full Relationship Intel brief (IntelBriefingCard) and the N/N Actions tracker (passed in via new `header` prop on DetailsTab), right above Relationship Voice Memos.
- Files: app/contact/[id].tsx (openSoldWizard extracted; SOLD bar + tracker + intel card removed from above tabs), components/contact/QuickActionsRow.tsx (onSold/showSold, filled style), components/contact/IntelTeaser.tsx (new), components/contact/IntelBriefingCard.tsx (export parseSections), components/contact/DetailsTab.tsx (header slot).
- Contact -> Inbox link (option a): gold chat-bubbles icon in the contact header next to Edit, sticky "Open conversation in Inbox" row at the top of the Feed tab (status line: Jessi handling / full conversation), and "Reply in Inbox" link inside expanded message events. `openInboxThread` in app/contact/[id].tsx finds-or-creates the conversation via messagesAPI.createConversation then routes to /thread/{id}. Composer mode row stays SMS / Email only (Inbox chip removed). Verified on preview.
