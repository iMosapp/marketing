...
## Speed-to-Lead + Photo Reminder + Monthly ROI Email (Aug 2026) — self-tested (curl + screenshots)
1. **Lead Response Timer**: `GET /api/leads/analytics/response-times` (per-rep avg/fastest/slowest first-human-reply time, batch aggregation over messages excluding AI/auto-sent). Leads list now returns `first_response_seconds` per lead. UI: new "Speed" tab on /leads (team avg + measured + no-reply cards, ranked rep leaderboard) + color-coded "Replied in Xm" badge on each lead card (green <5m, orange <1h, red >1h).
2. **Photo Reminder**: `list_inventory` counts include `missing_photos`; orange banner on /inventory when in-stock vehicles lack photos. Daily scheduler job (15:00 UTC) `send_missing_photo_reminders()` in inventory.py — push + in-app notification (idempotent per day) to store_managers/org_admins per store, or creator for storeless items.
3. **Monthly ROI Email**: scheduler job 1st of month 14:30 UTC → `send_monthly_roi_email()` in lead_intake.py. Per-store email (Resend) with per-source table: leads/replied/sold/spent/$-per-lead/$-per-sale. Recipient = store `roi_report_email` (new field in Admin → Store → AI & Security, GET/PUT ai-security-settings), fallback ADMIN_EMAIL/forest@imosapp.com. Test trigger: `POST /api/leads/analytics/roi-email-test` (auth). NOTE: preview Resend key domain unverified → sends fail in preview only; production key has send.imonsocial.com verified (same sender as working bug digest).
- Scheduler now runs 20 jobs (was 18): +daily_photo_reminder, +monthly_roi_email.

## Voice Memo Playback + Dialer Search/Layout Fixes (Aug 2026) — self-tested (curl + screenshots)
1. **Voice memo "Failed to play audio" ROOT CAUSE**: `create_voice_note` read `result.get("url")` from storage PUT response, but storage returns `path` — every voice note ever saved had EMPTY audio_url. Fixed: stores `audio_path` + `audio_url=/api/images/{path}`. Added `list_objects(prefix)` to image_storage.py. Backfill (`POST /api/voice-notes/admin/backfill-audio-urls`, super admin) matches orphaned notes to storage files per contact by timestamp — fixed 5/5 in preview. ALSO runs automatically at server startup (server.py `_voice_note_backfill`) so production self-heals on deploy. image_router `GET /{path}` now infers audio content types + supports HTTP Range (206) for iOS AVPlayer. Caveat: web-recorded `.webm` memos won't play on iOS natively (m4a ones fine) — transcoding is a possible follow-up.
2. **Dialer search found nothing**: dialer only loaded first 50 contacts (contactsAPI.getAll default limit) and filtered client-side. Now debounced (250ms) SERVER-side search via `/contacts/{uid}?search=` for both the name search box (2+ chars, digit queries stripped to digits) and keypad digit matching (3+ digits). Placeholder now "Search by name or number".
3. **Dialer layout overlap** (number over search bar/tabs, "Calling from" over search with keyboard open): keypad column was bottom-anchored and taller than available height → overflowed the top. Fixed: BTN_SIZE now capped by screen HEIGHT (`(SCREEN_H - 470 - 58)/5`, min 52), `overflow: hidden` guard on keypad column, and search-focused state shows the results panel instead of the keypad (nothing left to overlap the keyboard).

## Birthday/Anniversary Opt-In Overhaul (Aug 2026) — 7/7 engine tests passed (tests/test_date_optin_flow.py)
**Incident**: user edited contact birthdays → texts fired immediately (Jessi got TWO different AI birthday texts at 9:02 AM). ROOT CAUSE: (a) saving a birthday auto-applied the "Birthday" tag, (b) tag campaigns with trigger_tag=Birthday fired instantly on tag application, (c) save-time `_check_date_campaign_enrollment` ALSO enrolled with next_send_at=NOW regardless of actual date → double AI texts deferred to 9 AM by quiet hours.
**Fixes**:
1. contacts.py: NO auto Birthday/Anniversary tags on create/update (Sold Date marker kept). Tag campaigns SKIP birthday/anniversary trigger_tags (date sends never instant-fire). Save-time date enrollment removed (`_check_date_campaign_enrollment` is a no-op).
2. THE RULE: send only if contact has the date AND the manual opt-in tag ("Birthday" for birthdays; "Anniversary" for anniversary + sold-date anniversaries). Enforced in scheduler `_run_date_triggers_for_user` AND new `_enroll_date_campaigns_for_user` (day-of campaign enrollment in the 8AM UTC sweep; first matching campaign only; `date_send_guard` collection = one send per contact/occasion/day — doubles impossible). sold_date requires years>=1.
3. Sold-date anniversary: `{years}` template var + auto anniversary card link (card auto-uses the contact's car/delivery photo) appended to sold_date/anniversary trigger messages.
4. ONE-TIME startup migration `date_optin_reset_2026_08` (server.py): wipes ALL existing Birthday/Anniversary tags + cancels active birthday/anniversary/sold_date enrollments and their pending sends. Runs automatically on production deploy. Verified in preview.
...
3. **Webm Conversion**: voice_notes.py `_convert_webm_to_m4a` (imageio-ffmpeg static binary, AAC 64k) — new web recordings transcode at upload; `run_webm_conversion()` converts already-stored .webm notes at startup (idempotent, runs on production deploy). Verified: preview's webm memo now valid m4a (aac, decodes, serves audio/mp4 with Range support) → plays on iPhone.

## Keyword Auto-Tagging + Conversation Keyword Search (Aug 2026) — self-tested e2e
1. **Engine** (`services/keyword_tagging.py`): user-defined rules (keywords → tag). Scans OUTBOUND + INBOUND SMS (all 4 send/webhook paths in messages.py + twilio_webhooks.py) and CALL TRANSCRIPTS (recording-complete) — fire-and-forget. Word-boundary matching (multi-word phrases supported). Applies tag to contact ($addToSet), auto-creates tag doc, marks source message/call_log with `auto_tags`, records `keyword_tag_events` (contact, keyword, snippet, source_id, conversation_id — idempotent per source+tag). Forward-only (no retroactive scan, per user choice). No campaign auto-enrollment from keyword tags (intentional).
2. **Starter rules** seeded once per user (`keyword_rules_seeded` flag on users): Appointment, Trade, Financing, Test Drive, Price.
3. **API**: `/api/keyword-rules/{uid}` CRUD + `/events` (recent triggers w/ resolved message_id), `/api/search/{uid}/messages?q=` deep search across message content + call transcripts (also standalone call_logs), returns snippet + jump ids. Both added to PROTECTED_PREFIXES.
4. **UI**: `/settings/keyword-rules` (rules w/ hit counts, enable toggle, edit/delete, add modal w/ colors, Recent Auto-Tags feed → tap jumps to the exact message). `/keyword-search` screen (debounced, highlighted snippets, source icons, auto-tag chips, tap → thread jump). Entry points: More→Manage (2 rows), inbox search bar deep-search row, /search global deep-search row.
5. **In-thread search** (thread/[id].tsx): header magnifier toggles search bar — match counter (n/m), prev/next chevrons, yellow keyword highlight in bubbles, yellow border on current match, auto-scroll to match; auto-scroll-to-bottom suppressed while searching. Deep-link `?jumpToMsg=<id>&q=<term>` opens the bar and jumps straight to the message. Thread endpoint now returns transcript/call fields + auto_tags; auto-tag chips render under tagged bubbles.
- Verified: 5 starter rules seed, Gladiator custom rule, SMS send → contact tagged Appointment/Trade/Gladiator + events + chips, call-transcript path marks call_logs, search endpoint + all 3 screens + jump/highlight via screenshots.

## In-App Call Recording Player w/ Speed Control (Aug 2026) — self-tested e2e
1. **Backend** `GET /api/calls/recording/{call_sid}` (calls.py): streams the Twilio recording through the backend (Twilio basic auth server-side), /tmp cache keyed by recording_sid, full HTTP Range support (206 + Content-Range, open-ended ranges) so iOS/web can seek + change playback rate. call_sid is the capability token (unguessable, same model as media-proxy). Verified: 200 full, 206 bounded + open-ended ranges, 404 unknown sid, 502 on unfetchable recording.
2. **Component** `components/CallRecordingPlayer.tsx`: expo-av lazy-loaded player — play/pause, tap-to-seek progress bar, ±10s skip, time label, speed pill cycling 1x → 1.5x → 2x (`setRateAsync(rate, shouldCorrectPitch=true)`), playsInSilentModeIOS, error+retry state, unloads on unmount. Theme-able via tint/text/sub/track color props.
3. **Wired into**: thread/[id].tsx call card (replaced the external "Listen to Recording" link) and contact/[id].tsx Calls tab (player card above AI summary, direction-colored tint). URL: `/calls/recording/{call_sid}` primary, media-proxy fallback for logs missing call_sid.
- Verified in preview: player renders in Calls tab, plays (0:09/0:30 progressing), speed pill cycles to 2x, playback rate actually accelerates.

## Transcript Sync + Retro Scan + Keyword Alerts (Aug 2026) — self-tested e2e
1. **Transcript Sync**: `components/CallTranscript.tsx` (player + speaker transcript extracted from contact/[id].tsx Calls tab). CallRecordingPlayer gained `onPositionChange` + `seekControl` ref (`playFrom(ms)` — cold-loads sound at position if needed). Active segment (start <= pos) highlighted w/ tint bg + left border + bold; tap any timestamped line to jump audio there ("tap a line to jump" hint). Legacy flat transcripts render unchanged. Verified: highlight follows playback; tap-to-jump both directions (0:06 fwd, 0:00 back), cold-start tap works.
2. **Retro Scan**: `POST /api/keyword-rules/{uid}/{rule_id}/scan` → `scan_history_for_rule()` in keyword_tagging.py — broad mongo regex prefilter + python word-boundary verify across user's messages (content+transcript, 5000 cap) and standalone call_logs (1000 cap). Applies tags/events (flagged `retro_scan: True`)/auto_tags marks; call thread messages keyed by call_sid to avoid dup events; NEVER alerts. UI: clock button per rule w/ confirm + spinner + results alert. Verified: 15 msgs matched, 5 contacts tagged, idempotent.
3. **Keyword Alerts**: rules gained `alert_enabled` (bell toggle on row + "Instant alert" switch in modal, orange when on). Engine fires only on NEW events (upserted_id) from customer speech: source_type=call OR sender="contact" (send paths now pass sender; rep's own texts never alert). Sends db.notifications (type keyword_alert, priority high) + `send_push_to_user` (native+web, url→/thread/{conv}). Verified: alert created w/ 🔥 title + snippet, deduped on re-fire, outbound suppressed.
- Test data note: preview call_log CA1e4878e...643 was given demo transcript_segments + a cached 30s tone mp3 (/tmp/rec_cache_RE123test.mp3) for player/sync testing; Gladiator rule left with alert bell ON as demo.

## Ready-To-Buy Starter Rule + Team Keyword Rules (Aug 2026) — self-tested e2e
1. **Ready to Buy rule**: added to starter seed (alert_enabled=True, keywords: ready to buy/ready to purchase/coming in today/coming in now/on my way/buy today/where do i sign, color #FF3B30). `keyword_rules_seeded_v2` flag backfills it for already-seeded users on next rules fetch/engine run. Verified: Forest backfilled w/ alert ON.
2. **Team rules**: create_rule accepts scope="team" (ADMIN_ROLES: super_admin/org_admin/store_manager/admin) — attaches store_id (preferred) or org_id (fallback for admins w/o store, e.g. Forest). Engine + list now match user_id OR store_id OR org_id. `_can_manage()` guards PUT/DELETE (personal→owner, team→admin of same store/org); list returns `scope` + `editable` per rule. Verified: Forest created org-scoped "Hot Deal" team rule (left in DB as demo); rep PUT/DELETE on it → 403; store-scoped rule applied by engine for a rep in that store.
3. **UI** (settings/keyword-rules.tsx): "Just me / Whole team" scope pills in create modal (managers only), Team badge on team rule rows, edit/delete/toggle/bell hidden for non-editable team rules (scan always available — scans caller's own history), team-rule note shown when editing.

## Keyword Insights + Thread Refactor (Aug 2026) — self-tested e2e
1. **Keyword Insights**: `GET /api/keyword-rules/{uid}/insights` — aggregates keyword_tag_events (EXCLUDES retro_scan) into: this-week per-tag counts w/ top keyword + delta vs last week, totals, and 8-week trend buckets (Mon-start weeks). UI: "THIS WEEK" card at top of /settings/keyword-rules — big hit total w/ trend line vs last week, top-6 tag bars (rule-colored, width-scaled, delta arrows, top keyword), 8-week mini bar chart (current week purple). Hidden when zero activity both weeks. Verified: 3 hits rendered w/ correct tags/keywords/deltas.
2. **Thread Refactor**: extracted from thread/[id].tsx (5170→~4870 lines) into `components/thread/`: `CallLogCard.tsx` (call card + recording player), `MessageBubble.tsx` (full message render: rich-content detection, media/vcard, highlight, badges, auto-tag chips, date separator), `ThreadSearchBar.tsx` (in-thread search UI). renderMessage now ~28 lines delegating to them; CallRecordingPlayer import moved out of thread. Behavior verified identical via deep-link jump screenshot (bubbles, date sep, search 1/1, highlight, current-match border, auto-tags).
- Cleanup: stray ReviewTest auto_tags pulled from messages/call_logs (leftover from retro-scan test).

## Hub Simplification + My Profile Page (Aug 2026) — TESTED via testing_agent (iteration_285, ~93% pass, NO functional regressions)
1. **Hub (more.tsx)**: added hub-wide search bar (filters flat index of ALL items incl. admin+brand+profile, dedup by title, hides grid/sections while typing); 6-tile "What do you want to do?" task grid (Send a Blast /broadcast-message, Share My Card /quick-send/digitalcard, Get Reviews /quick-send/review, My Numbers /touchpoints/performance, Find a Word /keyword-search, Inventory /inventory); 'My Brand' accordion REMOVED (items live in /my-profile but stay searchable); 3 admin sections (setup_manage, account_mgmt, internal_ops) collapsed into ONE 'Admin' accordion (id admin_hub) w/ uppercase sub-group labels; count pills on all section headers. Profile card + button → /my-profile.
2. **NEW /my-profile.tsx**: cover header (cover photo or gold gradient), Edit My Info → /my-account, Virtual Assistant btn, profile-completion nudge → /profile-setup, HOW CUSTOMERS SEE ME: 5 public page cards (Digital Card /card/{id}, Link Page /l/{id}, Landing Page /p/{id}, Showcase /showcase/{id}, Review Link /review/{slug}?sp=) each w/ Share(copy)/Send to Contact(quick-send)/View(self_preview)/Edit; MY CONTENT: Templates (perm-gated), Card Templates, Email Signature, Quick Broadcast, Create a Card. Base URL = EXPO_PUBLIC_APP_URL env; store_slug falls back to user.store_slug then /admin/stores/{store_id} (parity w/ hub).
3. Post-test fixes applied: env-based PROD_BASE + store_slug fallback in my-profile.
- KNOWN pre-existing (NOT fixed, reported by tester): direct URL /inbox on preview serves marketing page (in-app tab nav fine); 1x1px profile thumb asset renders olive block avatar (same on /my-account before change); RNW strips data-testid attrs (use text selectors); stray text-node console warning on Hub/quick-send.

## Photo Upload + Next Action + Rep Preview + Weekly Digest (Aug 2026) — self-tested e2e
1. **Photo re-upload** (/my-profile): gold camera badge on avatar → web hidden-input / native ImagePicker (square crop) → POST /api/profile/{uid}/photo (existing pipeline: WebP + thumb + avatar) → setUser refreshes app-wide. Verified endpoint E2E w/ real image on mjeast account (Forest's broken 1x1 photo left for USER to re-upload).
2. **Home "DO THIS NEXT" hero** (home.tsx, above quick actions): single next action by priority: hotOpps[0] reply → overdue touchpoints → next incomplete My3 → pending touchpoints → fallback "ask for a review". Gold-bordered card w/ one CTA button (data-testid next-move-card). Verified: showed "Clear 138 overdue touchpoints".
3. **Rep Preview** (more.tsx): admins get "View as Rep" text button under profile buttons → repPreview state gates isAdmin/isSuperAdmin/isStoreManager/isPartner/canManageCampaigns → Admin accordion + Partner Portal + role-gated items vanish; gold banner "Rep Preview — tap to exit". Verified via screenshot (sections and counts reduced correctly).
4. **Weekly keyword digest SMS** (scheduler.py `send_weekly_keyword_digests`, hourly cron :10): Monday 8 AM user-local (pytz, default America/Denver), guarded by date_send_guard kwdigest_{uid}_{ISO week}, aggregates LAST week's keyword_tag_events (excl retro_scan) → SMS top-5 tags w/ top keyword + call count via send_sms from user's twilio number + idempotent in-app notification (link /settings/keyword-rules). Skips users w/ no events. Verified: function runs clean; composition dry-run produced correct SMS body.

## Contact Page Refactor (Aug 21, 2026) — COMPLETE, self-tested
- `app/contact/[id].tsx` reduced 5,288 → 2,985 lines; JSX extracted into 12 prop-driven components in `components/contact/` (HeroSection, EditFormTop/Bottom, FeedTab, DetailsTab, CallsTab, ComposerBar, ShareModals, PickerModals, DateModals, AddTaskModal, GalleryModal). No functional/visual change intended or observed.
- Note for future agents: RN-web does NOT expose raw `data-testid` props on Touchable/View in the DOM for this screen (pre-existing). Playwright must locate by text/coordinates on contact detail.
